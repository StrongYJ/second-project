...
//    @Query("select SUBSTRING(cast(i.date as text), 1,7) as yearMonth, avg(i.amount) as avg from IncomeInfo i where i.member = :member and i.date between :start and :end group by yearMonth order by yearMonth desc")
    ...
    @Query("select i from IncomeInfo i where i.member = :member and date_format(date, '%y%m') = :date")
    // @Query("select i from IncomeInfo i where i.member = :member and i.date between :start and :end order by i.date")
    List<IncomeInfo> findByYearMonth(@Param("member") MemberInfo member, @Param("date") Integer date);

    @Query("select sum(i.amount)as sum, SUBSTRING(cast(i.date as text), 1,7) as yearMonth, rank() over(order by sum(i.amount) desc) as rank from IncomeInfo i where i.member = :member and SUBSTRING(cast(i.date as text), 1,4) = :year group by yearMonth")
